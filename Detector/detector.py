...
class Detector:
    def __init__(self, model_type: str, device, image_size, output_size):
        ...
        if model_type == 'mtcnn':
            ...
            self.detector = YOLO('yolov8n-face.pt', verbose=False)
        elif model_type=='yolom':
            self.detector = YOLO('yolov8m-face.pt', verbose=False)
        elif model_type=='yolol':
            self.detector = YOLO('yolov8l-face.pt', verbose=False)
        else:
...
st= time.time()
boxes, faces = detector.detect_with_extractor(x)
print(time.time()-st)