...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--target", help="verification targets", default='lfw,cfp_fp,agedb_30', type=str)
    ...
    BACKBONE = CoAtNet(num_classes=NUM_CLASS)
    ...
    OPTIMIZER = timm.optim.create_optimizer(args, BACKBONE)
    print()
    ...
    if True:
        ...
        if True:
            print("Loading Backbone Checkpoint '{}'".format("Backbone_accuracy_[0.9946666666666667]_epochs1_batch_246400_checkpoint.pth"))
            BACKBONE.load_state_dict(torch.load("model/Backbone_accuracy_[0.9951666666666666]_epochs1_batch_188800_checkpoint.pth"), strict=False)
        print("=" * 60)
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for inputs, labels in iter(trainloader):
            ...
            prec1= train_accuracy(outputs, labels, topk = (1,))
            ...
            if ((batch + 1) % VER_FREQ == 0) and batch != 0:
                ...
                for ver in vers:
                    ...
                    accuracy, std, xnorm, best_threshold, roc_curve, accuracy1 = perform_val(MULTI_GPU, DEVICE, EMBEDDING_SIZE, BATCH_SIZE, BACKBONE, data_set, issame, 15)
                ...
                if need_save(acc, highest_acc):
                    if MULTI_GPU:
                        torch.save(BACKBONE.module.state_dict(), os.path.join(WORK_PATH, "Backbone_{}_Epoch_{}_Batch_{}_Time_{}_checkpoint.pth".format(BACKBONE_NAME, epoch + 10, batch + 1, get_time())))
                    else:
                        torch.save(BACKBONE.state_dict(), os.path.join(WORK_PATH, "Backbone_accuracy_{}_epochs{}_batch_{}_checkpoint.pth".format(acc, epoch+1, batch+1)))
                BACKBONE.train()
