...
def get_val_pair(path, name):
    ...
    data_set, issame = load_bin(ver_path, [224,224])
    print('ver', name)
...
def perform_val(multi_gpu, device, embedding_size, batch_size, backbone, data_set, issame, nrof_folds = 5):
    if multi_gpu:
    ...
    for carray in data_set:
        ...
        with torch.no_grad():
            while idx + batch_size <= len(carray):
                ...
                #last_time = time.time()
                embeddings[idx:idx + batch_size] = backbone(transform(batch).to(device)).cpu()
                #batch_time = time.time() - last_time
                #print("batch_time", batch_size, batch_time)
                idx += batch_size
            if idx < len(carray):
                batch = carray[idx:]
                embeddings[idx:] = backbone(transform(batch).to(device)).cpu()
    ...
    tpr, fpr, accuracy, best_thresholds = evaluate(embeddings, issame, nrof_folds=nrof_folds)
    buf = gen_plot(fpr, tpr)
    roc_curve = Image.open(buf)
    roc_curve_tensor = transforms.ToTensor()(roc_curve)

    return accuracy.mean(), accuracy.std(), _xnorm, best_thresholds.mean(), roc_curve_tensor, accuracy
